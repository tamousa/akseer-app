...
export default function AddRehabOffer() {
  ...
  const save = () => {
    if (!name.trim() || !discVal.trim() || !expiry.trim()) return Alert.alert(t("تنبيه","Notice"), t("يرجى إكمال البيانات المطلوبة","Please complete the required data"));
    Alert.alert(t("تم الحفظ ✓","Saved ✓"), `${t("عرض:","Offer:")} ${name}\n${t("الكود:","Code:")} ${code || t("بدون كود","No code")}`, [{ text: t("موافق","OK"), onPress: () => router.back() }]);
  };
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={{ flex: 1, backgroundColor: bg }}>
        <View style={[st.hdr, { backgroundColor: BRAND, paddingTop: isWeb ? 72 : insets.top + 16 }]}>
          ...
          <Text style={st.hdrTitle}>{t("إضافة عرض تأهيل","Add Rehab Offer")}</Text>
        ...
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 80, gap: 14 }}>
          <View style={[st.sec, { backgroundColor: card, borderColor: BRAND + "20" }]}>
            ...
            <View style={[st.inpWrap, { borderColor: colors.border }]}>
              <TextInput style={[st.inp, { color: colors.text }]} placeholder={t("اسم العرض *","Offer Name *")} placeholderTextColor={colors.muted} value={name} onChangeText={setName} textAlign="right" />
            </View>
          </View>
          <View style={[st.sec, { backgroundColor: card, borderColor: BRAND + "20" }]}>
            <Text style={[st.secTitle, { color: colors.text }]}>{t("نوع الخصم وقيمته","Discount Type & Value")}</Text>
          ...
          <View style={[st.sec, { backgroundColor: card, borderColor: BRAND + "20" }]}>
            ...
            {code && (
              <View style={{ padding: 14, borderRadius: 12, backgroundColor: isDark ? "#003820" : "#D1FAE5", alignItems: "center" }}>
