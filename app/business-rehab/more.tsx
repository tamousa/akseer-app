...
export default function RehabMore() {
  ...
  const ITEMS = [
    { labelAr: "الفريق الطبي", labelEn: "Medical Team", icon: "users" as const, route: "/business-rehab/staff", color: BRAND },
    { labelAr: "خطط العلاج", labelEn: "Treatment Plans", icon: "clipboard" as const, route: "/business-rehab/treatment-plans", color: BRAND },
    { labelAr: "العروض والباقات", labelEn: "Offers & Packages", icon: "tag" as const, route: "/business-rehab/offers", color: "#047857" },
    { labelAr: "مطالبات التأمين", labelEn: "Insurance Claims", icon: "shield" as const, route: null, color: "#6366F1" },
    { labelAr: "معاينة الصفحة", labelEn: "Page Preview", icon: "eye" as const, route: "/business-rehab/rehab-preview", color: "#047857" },
    { labelAr: "التقارير والإحصاء", labelEn: "Reports & Statistics", icon: "bar-chart-2" as const, route: null, color: "#6366F1" },
    { labelAr: "إعدادات الحساب", labelEn: "Account Settings", icon: "settings" as const, route: null, color: "#9CA3AF" },
    { labelAr: "الدعم والمساعدة", labelEn: "Support & Help", icon: "help-circle" as const, route: null, color: "#F59E0B" },
  ];
  ...
  return (
    <View style={[s.container, { backgroundColor: isDark ? "#001A12" : "#ECFDF5" }]}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 80, gap: 10 }}>
        {ITEMS.map((item, i) => (
          <Pressable key={i} style={[s.item, { backgroundColor: isDark ? "#003020" : "#fff", borderColor: BRAND + "25" }]}
