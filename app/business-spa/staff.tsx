...
export default function SpaStaff() {
  ...
  const STAFF = [
    { id: 1, nameAr: "أحمد الزهراني", nameEn: "Ahmed Al-Zahrani", specialtyAr: "تدليك سويدي وأحجار", specialtyEn: "Swedish & Stone Massage", sessions: 560, rating: 4.9, statusAr: "نشط", statusEn: "Active", color: BRAND },
    { id: 2, nameAr: "وليد العمري", nameEn: "Walid Al-Omari", specialtyAr: "رياضي وعلاج طبيعي", specialtyEn: "Sports & Physiotherapy", sessions: 380, rating: 4.8, statusAr: "نشط", statusEn: "Active", color: "#4F46E5" },
    { id: 3, nameAr: "سامي الشهري", nameEn: "Sami Al-Shahri", specialtyAr: "تدليك عام وأروماثيرابي", specialtyEn: "General & Aromatherapy", sessions: 290, rating: 4.7, statusAr: "إجازة", statusEn: "On Leave", color: "#7C3AED" },
  ];
  ...
  return (
    <View style={[s.container, { backgroundColor: colors.background }]}>
      <View style={[s.header, { backgroundColor: colors.surface, paddingTop: isWeb ? 72 : insets.top + 16, borderBottomWidth: 1, borderBottomColor: colors.border }]}>
        <Text style={[s.headerTitle, { color: colors.text }]}>{t("المعالجون","Therapists")} ({STAFF.length})</Text>
        <Pressable style={[s.addBtn, { backgroundColor: colors.surfaceAlt }]} onPress={() => Alert.alert(t("إضافة معالج","Add Therapist"), t("سيتم فتح نموذج إضافة معالج جديد","A new therapist form will open"))}>
          <Feather name="user-plus" size={20} color={colors.primary} />
        </Pressable>
      </View>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 90, gap: 10 }}>
        {STAFF.map((m) => (
          <View key={m.id} style={[s.card, { backgroundColor: isDark ? "#12124A" : "#fff", borderColor: m.color + "30" }]}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[s.role, { color: colors.muted }]}>{lang === "ar" ? m.specialtyAr : m.specialtyEn}</Text>
