...
const SVCS = [
  { id: "1", nameAr: "تدليك استرخاء كامل", nameEn: "Full Relaxation Massage", price: 320 },
  { id: "2", nameAr: "جلسة حرارة بالحجارة", nameEn: "Hot Stone Session", price: 280 },
  { id: "3", nameAr: "علاج بالأعشاب", nameEn: "Herbal Treatment", price: 180 },
  { id: "4", nameAr: "تدليك الوجه", nameEn: "Facial Massage", price: 150 },
  { id: "5", nameAr: "جاكوزي خاص", nameEn: "Private Jacuzzi", price: 200 },
];

const INIT = [
  { id: "1", nameAr: "باقة الاسترخاء الملكية", nameEn: "Royal Relaxation Package", svcs: ["1","2","5"], price: 680, orig: 800, expiry: "31/12/2026", active: true },
  { id: "2", nameAr: "باقة العلاج الطبيعي", nameEn: "Natural Therapy Package", svcs: ["3","4"], price: 300, orig: 330, expiry: "30/09/2026", active: true },
];
...
export default function SpaPackages() {
  ...
  const add = () => {
    ...
    setPkgs((p) => [{ id: Date.now().toString(), nameAr: nm, nameEn: nm, svcs: sel, price: parseFloat(pr), orig, expiry: ex || t("بلا انتهاء","No expiry"), active: true }, ...p]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: bg }}>
      <View style={[st.hdr, { backgroundColor: colors.surface, paddingTop: isWeb ? 72 : insets.top + 16, borderBottomWidth: 1, borderBottomColor: colors.border }]}>
        ...
        <Text style={st.hdrTitle}>{t("باقات السبا","Spa Packages")}</Text>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 90, gap: 14 }}>
        {adding && (
          <View style={[st.sec, { backgroundColor: card, borderColor: BRAND + "40", borderWidth: 2 }]}>
            ...
            <Text style={{ fontSize: 12, fontFamily: "Tajawal_700Bold", color: BRAND, textAlign: "right" }}>{t("الخدمات المشمولة:","Included Services:")}</Text>
            {SVCS.map((s) => {
            ...
            {sel.length > 0 && parseFloat(pr) > 0 && (
              <View style={{ padding: 12, borderRadius: 12, backgroundColor: isDark ? "#0E0E3A" : "#EEF2FF", gap: 4 }}>
                <View style={{ flexDirection: "row-reverse", justifyContent: "space-between" }}><Text style={{ fontSize: 20, fontFamily: "Cairo_700Bold", color: BRAND }}>{parseFloat(pr)} SAR</Text><Text style={{ fontSize: 12, fontFamily: "Tajawal_400Regular", color: colors.muted }}>{t("السعر الجديد","New Price")}</Text></View>
                <View style={{ flexDirection: "row-reverse", justifyContent: "space-between" }}><Text style={{ fontSize: 14, fontFamily: "Cairo_700Bold", color: "#059669" }}>{t("وفّر","Save")} {orig - parseFloat(pr)} SAR</Text><Text style={{ fontSize: 12, fontFamily: "Tajawal_400Regular", color: colors.muted, textDecorationLine: "line-through" }}>{orig} SAR</Text></View>
              </View>
            )}
