...
export default function AddSpaOffer() {
  ...
  const save = () => {
    ...
    Alert.alert(t("تم الحفظ ✓","Saved ✓"), `${t("عرض:","Offer:")} ${offerName}`, [{ text: t("موافق","OK"), onPress: () => router.back() }]);
  };
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={{ flex: 1, backgroundColor: bg }}>
        <View style={[st.hdr, { backgroundColor: BRAND, paddingTop: isWeb ? 72 : insets.top + 16 }]}>
          ...
          <Text style={st.hdrTitle}>{t("إضافة عرض سبا","Add Spa Offer")}</Text>
        ...
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 80, gap: 14 }}>
          <View style={[st.sec, { backgroundColor: card, borderColor: BRAND + "20" }]}>
            ...
            {[
              { ph: t("اسم العرض *","Offer Name *"), val: offerName, set: setOfferName },
          ...
          <View style={[st.sec, { backgroundColor: card, borderColor: BRAND + "20" }]}>
            ...
            {[
              { ph: discType === "pct" ? t("الخصم (مثال: 25)","Discount (e.g. 25)") : t("مبلغ الخصم","Discount Amount"), val: discVal, set: setDiscVal, suf: discType === "pct" ? "%" : "SAR" },
              { ph: t("الحد الأدنى للطلب","Min. Order"), val: minOrder, set: setMinOrder, suf: "SAR" },
          ...
          <View style={[st.sec, { backgroundColor: card, borderColor: BRAND + "20" }]}>
            ...
            <View style={[st.inpWrap, { borderColor: code ? BRAND : colors.border, borderWidth: code ? 2 : 1 }]}>
              <TextInput style={[st.inp, { color: colors.text, fontFamily: "Cairo_700Bold", letterSpacing: 1 }]} placeholder={t("كود مخصص أو اضغط توليد","Custom code or tap generate")} placeholderTextColor={colors.muted} value={code} onChangeText={(v) => setCode(v.toUpperCase())} textAlign="right" autoCapitalize="characters" />
            ...
            <Pressable style={{ flexDirection: "row-reverse", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 12, borderRadius: 12, borderWidth: 1, backgroundColor: BRAND + "15", borderColor: BRAND + "50" }} onPress={() => setCode(genCode())}>
              <Feather name="zap" size={16} color={BRAND} /><Text style={{ fontSize: 13, fontFamily: "Cairo_700Bold", color: BRAND }}>{t("توليد كود تلقائي","Auto-generate Code")}</Text>
            </Pressable>
            {code && (
              <View style={{ padding: 14, borderRadius: 12, backgroundColor: isDark ? "#0E0E3A" : "#EEF2FF", alignItems: "center" }}>
