...
export default function BeautyMore() {
  ...
  const ITEMS = [
    { labelAr: "الموظفات", labelEn: "Staff", icon: "users" as const, route: "/business-beauty/staff", color: BRAND },
    { labelAr: "العروض والخصومات", labelEn: "Offers & Discounts", icon: "tag" as const, route: "/business-beauty/offers", color: BRAND },
    { labelAr: "معرض الأعمال", labelEn: "Portfolio", icon: "image" as const, route: "/business-beauty/gallery", color: "#DB2777" },
    { labelAr: "معاينة الصفحة", labelEn: "Page Preview", icon: "eye" as const, route: "/business-beauty/beauty-preview", color: "#9D174D" },
    { labelAr: "التقييمات", labelEn: "Reviews", icon: "star" as const, route: null, color: "#F59E0B" },
    { labelAr: "التقارير", labelEn: "Reports", icon: "bar-chart-2" as const, route: null, color: "#6366F1" },
    { labelAr: "إعدادات الحساب", labelEn: "Account Settings", icon: "settings" as const, route: null, color: "#78350F" },
    { labelAr: "الدعم والمساعدة", labelEn: "Support & Help", icon: "help-circle" as const, route: null, color: "#059669" },
  ];
  ...
  return (
    <View style={[s.container, { backgroundColor: isDark ? "#150010" : "#FFF0F6" }]}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 80, gap: 10 }}>
        {ITEMS.map((item, i) => (
          <Pressable key={i} style={[s.item, { backgroundColor: isDark ? "#2D0020" : "#fff", borderColor: BRAND + "25" }]}
