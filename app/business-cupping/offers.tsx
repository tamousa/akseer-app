  ...
  return (
    <View style={[s.container, { backgroundColor: colors.background }]}>
      <View style={[s.header, { backgroundColor: colors.surface, paddingTop: isWeb ? 72 : insets.top + 16, borderBottomWidth: 1, borderBottomColor: colors.border }]}>
        <Text style={[s.headerTitle, { color: colors.text }]}>{t("العروض والباقات","Offers & Packages")}</Text>
        <View style={{ flexDirection: "row-reverse", gap: 8 }}>
          <Pressable style={[s.addBtn, { backgroundColor: colors.surfaceAlt }]} onPress={() => router.push("/business-cupping/packages" as any)}>
            <Feather name="package" size={18} color={colors.primary} />
          </Pressable>
          <Pressable style={[s.addBtn, { backgroundColor: colors.surfaceAlt }]} onPress={() => router.push("/business-cupping/add-offer" as any)}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 90, gap: 10 }}>
        {OFFERS.map((o) => (
          <View key={o.id} style={[s.card, { backgroundColor: colors.surface, borderColor: BRAND + "25" }]}>
            <View style={{ flex: 1 }}>
