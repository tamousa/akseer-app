...
export default function CuppingMore() {
  ...
  const ITEMS = [
    { labelAr: "المعالجون", labelEn: "Therapists", icon: "users" as const, route: "/business-cupping/staff", color: BRAND },
    { labelAr: "العروض والباقات", labelEn: "Offers & Packages", icon: "tag" as const, route: "/business-cupping/offers", color: BRAND },
    { labelAr: "أنواع الجلسات", labelEn: "Session Types", icon: "droplet" as const, route: "/business-cupping/sessions", color: "#B45309" },
    { labelAr: "سجل التعقيم", labelEn: "Sterilization Log", icon: "shield" as const, route: null, color: "#059669" },
    { labelAr: "الزيارات المنزلية", labelEn: "Home Visits", icon: "home" as const, route: null, color: "#6366F1" },
    { labelAr: "معاينة الصفحة", labelEn: "Page Preview", icon: "eye" as const, route: "/business-cupping/cupping-preview", color: "#78350F" },
  ...
  return (
    <View style={[s.container, { backgroundColor: isDark ? "#1A0E00" : "#FFFBEB" }]}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 80, gap: 10 }}>
        {ITEMS.map((item, i) => (
          <Pressable key={i} style={[s.item, { backgroundColor: isDark ? "#2A1500" : "#fff", borderColor: BRAND + "25" }]}
